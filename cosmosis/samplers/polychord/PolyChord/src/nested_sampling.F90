module nested_sampling_module
    ...
    abstract interface
        subroutine cosmosis_output_sub_type(weight, post, n, params)
            ...
            real(c_double), dimension(n) ::  params

        end subroutine cosmosis_output_sub_type
    end interface
    ...
    function NestedSampling(loglikelihood,priors,settings,mpi_communicator, cosmosis_output_sub_c) result(output_info)
        use iso_c_binding
        ...
        type(c_funptr), intent(in), value :: cosmosis_output_sub_c
        ...
        !    ==============
        ! MPI initialisation
        call c_f_procpointer(cosmosis_output_sub_c, cosmosis_output_sub)
                ...
                if( linear_mode(mpi_information) .or. master_epoch==slave_epoch ) then
                    ...
                    if(replace_point(settings,RTI,baby_points,cluster_id) ) then
                        ...
                        if( cyc(RTI%ndead,settings%update_files) .or. .not. need_more_samples ) then
                            ...
                            if(settings%equals.or.settings%posteriors) call write_posterior_file_cosmosis(settings,RTI, cosmosis_output_sub)   
    ...
    subroutine write_posterior_file_cosmosis(settings,RTI, cosmosis_output_sub)
